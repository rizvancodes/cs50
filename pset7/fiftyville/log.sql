...
SELECT * FROM people WHERE passport_number = (SELECT passport_number FROM passengers JOIN flights ON passengers.flight_id = flights.id); WHERE year = 2021 AND month = 7 AND day = 29; (SELECT passport_number FROM people WHERE phone_number = (SELECT caller FROM phone_calls WHERE year = 2021 AND month = 7 AND day = 28));